...
async def get_memory(tenant_id: str, _user: CurrentUser, svc: AnalysisService = Depends(_svc)):
    return await svc.get_tenant_memory(tenant_id)


@router.delete("/{tenant_id}")
async def delete_memory(tenant_id: str, _user: CurrentUser, svc: AnalysisService = Depends(_svc)):
    deleted = await svc.delete_tenant_memory(tenant_id)
    return {"status": "ok", "deleted": deleted}


@router.delete("/{tenant_id}/last")
async def delete_last(tenant_id: str, _user: CurrentUser, svc: AnalysisService = Depends(_svc)):
