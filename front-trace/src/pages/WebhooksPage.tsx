      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">Configura a donde enviar eventos cuando algo pasa en Trace</p>
          </div>
        </div>
              ...
              <div className="px-5 py-4 flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={cn('text-[10px] px-1.5 py-0.5 rounded-full font-medium', s.is_active ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-500')}>
                      {s.is_active ? 'Activo' : 'Inactivo'}
                    </span>
                  </div>
                  <p className="text-xs text-gray-400 font-mono truncate mt-0.5">{s.target_url}</p>
                  <div className="flex gap-1.5 mt-1.5 flex-wrap">
                    {s.events.length === 0 ? (
                      <span className="text-[10px] px-1.5 py-0.5 bg-gray-100 text-gray-500 rounded">Todos los eventos</span>
                    ) : s.events.map(ev => (
                ...
                <div className="text-right shrink-0">
                  {s.secret && <p className="text-[10px] font-mono text-gray-400">{s.secret}</p>}
                  {s.last_triggered_at && <p className="text-[10px] text-gray-400 mt-0.5">Ultimo: {new Date(s.last_triggered_at).toLocaleString('es-CO')}</p>}
                </div>
                ...
                <div className="flex items-center gap-1 shrink-0">
                  ...
                  <button onClick={() => handleToggle(s)} title={s.is_active ? 'Desactivar' : 'Activar'}
                    className={cn('p-1.5 rounded-lg', s.is_active ? 'text-gray-400 hover:text-amber-600 hover:bg-amber-50' : 'text-gray-400 hover:text-emerald-600 hover:bg-emerald-50')}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => { setShowCreate(false); setEditSub(null) }}>
          <div className="bg-white rounded-2xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="text-xs text-gray-500">URL destino * (HTTPS recomendado)</label>
              ...
              <div>
                <label className="text-xs text-gray-500">Reintentos maximos</label>
              ...
              {/* Events selector */}
              <div>
                <label className="text-xs text-gray-500 mb-2 block">Eventos (dejar vacio = todos)</label>
                        ...
                        return (
                          <button key={item.event} type="button"
                            ...
                            className={cn(
                              ...
                              selected ? 'bg-indigo-600 text-white border-indigo-600' : `${SOURCE_COLORS[source] ?? 'bg-gray-50 text-gray-600 border-gray-200'} hover:border-indigo-300`,
                            )}>
                            {item.label}
                          </button>
                        )
            ...
            <div className="flex gap-3 mt-6">
              <button onClick={() => { setShowCreate(false); setEditSub(null); resetForm() }}
                className="flex-1 bg-gray-100 text-gray-700 rounded-xl px-4 py-2.5 text-sm">Cancelar</button>
...
function DeliveriesPanel({ subId }: { subId: string }) {
  ...
  if (isLoading) return <div className="px-5 py-4 border-t text-center text-gray-400 text-sm">Cargando entregas...</div>

  if (!deliveries || deliveries.length === 0) {
    return <div className="px-5 py-4 border-t text-center text-gray-400 text-sm">Sin entregas registradas</div>
  }
        ...
        <tbody>
          {deliveries.slice(0, 20).map(d => (
            <tr key={d.id} className="border-t border-gray-50 hover:bg-gray-50/50">
              ...
              <td className="px-4 py-1.5 text-gray-400">{new Date(d.created_at).toLocaleString('es-CO')}</td>
            </tr>
          ))}
        </tbody>
      </table>
