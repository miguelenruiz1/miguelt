  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">Mano de obra, maquinaria y costos indirectos de produccion</p>
          </div>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16 text-gray-400">Sin recursos — crea mano de obra o maquinaria para asignar a recetas</div>
      ) : (
                  ...
                  <tr key={r.id} className="border-b border-gray-50 hover:bg-gray-50/50">
                    ...
                    <td className="px-4 py-2.5 text-right">
                      <button onClick={() => { setForm({ name: r.name, resource_type: r.resource_type, cost_per_hour: r.cost_per_hour, capacity_hours_per_day: r.capacity_hours_per_day, efficiency_pct: r.efficiency_pct, shifts_per_day: String(r.shifts_per_day), notes: r.notes ?? '' }); setEditId(r.id); setShowCreate(true) }}
                        className="p-1 text-gray-400 hover:text-amber-600"><Pencil className="h-3.5 w-3.5" /></button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => { setShowCreate(false); setEditId(null) }}>
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="col-span-2"><label className="text-xs text-gray-500">Nombre *</label><input value={form.name} onChange={e => setForm(f => ({...f, name: e.target.value}))} className={inputCls} /></div>
              ...
              <div><label className="text-xs text-gray-500">Costo/hora ($)</label><input type="number" step="0.01" min="0" value={form.cost_per_hour} onChange={e => setForm(f => ({...f, cost_per_hour: e.target.value}))} className={inputCls} /></div>
              <div><label className="text-xs text-gray-500">Horas/dia</label><input type="number" step="0.5" min="0" value={form.capacity_hours_per_day} onChange={e => setForm(f => ({...f, capacity_hours_per_day: e.target.value}))} className={inputCls} /></div>
              <div><label className="text-xs text-gray-500">Eficiencia %</label><input type="number" min="0" max="100" value={form.efficiency_pct} onChange={e => setForm(f => ({...f, efficiency_pct: e.target.value}))} className={inputCls} /></div>
              <div><label className="text-xs text-gray-500">Turnos/dia</label><input type="number" min="1" value={form.shifts_per_day} onChange={e => setForm(f => ({...f, shifts_per_day: e.target.value}))} className={inputCls} /></div>
            ...
            <div className="flex gap-3 mt-4">
              <button onClick={() => { setShowCreate(false); setEditId(null); resetForm() }} className="flex-1 bg-gray-100 text-gray-700 rounded-xl px-4 py-2.5 text-sm">Cancelar</button>
