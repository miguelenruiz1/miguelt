...
function NoSubscription() {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-24 px-4">
      ...
      <h2 className="text-xl font-bold text-slate-800 mb-2">No tienes suscripciones activas</h2>
...
export function SubscriptionDetailPage() {
  ...
  if (!sub) {
    return (
      <div className="p-8">
        <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-sm text-slate-500 hover:text-slate-800 transition-colors mb-6">
    ...
    <div className="p-8 space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
          ...
          <p className="text-xs text-slate-400 mt-1">{CYCLE_LABELS[sub.billing_cycle] ?? sub.billing_cycle}</p>
        </div>
        ...
        <div className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
          ...
          <p className="text-2xl font-bold text-slate-900">{days} dias</p>
      ...
      <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
        <h3 className="text-sm font-semibold text-slate-700 mb-3">Limites del plan</h3>
      ...
      {cancelConfirm && (
        <div className="rounded-xl border border-red-200 bg-red-50 p-4 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button onClick={() => setCancelConfirm(false)} className="rounded-lg px-3 py-1.5 text-sm text-slate-600 hover:bg-white">No</button>
            <button
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50">
                  <th className="px-4 py-3 text-left font-semibold text-slate-500">Numero</th>
                  <th className="px-4 py-3 text-left font-semibold text-slate-500">Estado</th>
                  <th className="px-4 py-3 text-left font-semibold text-slate-500">Monto</th>
                  <th className="px-4 py-3 text-left font-semibold text-slate-500">Periodo</th>
                  <th className="px-4 py-3 text-left font-semibold text-slate-500">Accion</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-50">
                ...
                {invoices.length === 0 && (
                  <tr><td colSpan={5} className="px-4 py-8 text-center text-slate-400 text-sm">Sin facturas</td></tr>
                )}
              </tbody>
            </table>
          </div>
      ...
      {tab === 'events' && (
        <div className="space-y-3">
          {events.map(ev => (
            <div key={ev.id} className="flex items-start gap-4 rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
              <span className="text-2xl">{EVENT_ICONS[ev.event_type] ?? '📌'}</span>
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <span className="text-sm font-semibold text-slate-800 capitalize">{ev.event_type.replace(/_/g, ' ')}</span>
                  {ev.performed_by && (
                    <span className="text-xs text-slate-400">por {ev.performed_by}</span>
                  )}
                </div>
                <p className="text-xs text-slate-500 mt-0.5">{new Date(ev.created_at).toLocaleString()}</p>
                {ev.data && Object.keys(ev.data).length > 0 && (
                  <pre className="mt-2 text-xs text-slate-600 bg-slate-50 rounded-lg p-2 overflow-x-auto">
