...
export function PlatformTenantsPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">Empresas Registradas</h1>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        {data && (
          <span className="text-sm text-slate-500">{data.total} empresa{data.total !== 1 ? 's' : ''}</span>
        )}
      </div>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="text-xs text-slate-400">{t.billing_cycle}</div>
                    </div>
                  ...
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <span className="font-medium text-slate-700">{t.plan.name} <span className="text-slate-400">${t.plan.price_monthly}/mes</span></span>
                  </div>
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-slate-500">Modulos</span>
                    <div className="flex flex-wrap gap-1 justify-end">
                      {t.active_modules.length > 0 ? t.active_modules.map(m => (
                        <span key={m} className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-xs font-medium ${MODULE_COLORS[m] ?? 'bg-slate-100 text-slate-600'}`}>
                  ...
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <div className="text-right">
                      <span className="font-semibold text-slate-900">${t.total_revenue.toLocaleString()}</span>
                      <span className="text-slate-400 ml-1">({t.invoice_count} facturas)</span>
                    </div>
                  </div>
                  {t.created_at && (
                    <div className="flex items-center justify-between text-xs">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-100">
                  ...
                  <th className="text-left px-5 py-3 font-semibold text-slate-600">Modulos</th>
                  <th className="text-right px-5 py-3 font-semibold text-slate-600">Ingresos</th>
                  <th className="text-left px-5 py-3 font-semibold text-slate-600">Registro</th>
                  ...
                  return (
                    <tr key={t.tenant_id} className="hover:bg-slate-50/60 transition">
                      ...
                      <td className="px-5 py-4">
                        ...
                        <div className="text-xs text-slate-400">${t.plan.price_monthly}/mes</div>
                      </td>
                      <td className="px-5 py-4">
                        <span className={`inline-flex px-2.5 py-1 rounded-lg text-xs font-semibold ${badge.bg} ${badge.text}`}>
                          {badge.label}
                      ...
                      <td className="px-5 py-4 text-right">
                        <div className="font-semibold text-slate-900">${t.total_revenue.toLocaleString()}</div>
