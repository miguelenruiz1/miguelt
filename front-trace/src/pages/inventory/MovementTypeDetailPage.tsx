...
const DIRECTION_LABELS: Record<string, { label: string; color: string }> = {
  ...
  neutral: { label: 'Neutral', color: 'bg-secondary text-muted-foreground' },
}
    ...
    <form onSubmit={submit} className="space-y-6 max-w-xl">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="space-y-3 pt-5">
            <label className="flex items-center gap-2 text-sm cursor-pointer">
              ...
              <span className="text-xs font-medium text-muted-foreground">Afecta costo</span>
            </label>
            <label className="flex items-center gap-2 text-sm cursor-pointer">
              <input type="checkbox" checked={form.requires_reference} disabled={disabled}
                ...
                className="rounded disabled:opacity-50" />
              <span className="text-xs font-medium text-muted-foreground">Requiere referencia</span>
            </label>
          </div>
        </div>
        ...
        <div className="rounded-lg bg-muted px-4 py-3">
          ...
          <code className="text-sm font-mono text-foreground">{movementType.slug}</code>
        </div>
      </div>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">Campos exclusivos de este tipo de movimiento</p>
      ...
      {isLoading ? (
        ...
        <div className="rounded-xl border-2 border-dashed border-border p-10 text-center">
          ...
          <p className="text-xs text-muted-foreground mb-4">Crea campos exclusivos para este tipo de movimiento.</p>
          <button onClick={openCreate}
  ...
  return (
    <div className="p-8 space-y-6 max-w-5xl mx-auto">
      ...
      <nav className="flex items-center gap-1.5 text-xs text-muted-foreground">
        ...
        <Link to="/inventario/configuracion/tipos-movimiento" className="hover:text-muted-foreground transition-colors">Tipos de movimiento</Link>
        <ChevronRight className="h-3 w-3" />
        <span className="text-foreground font-medium">{movementType.name}</span>
      </nav>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2">
            <h1 className="text-xl font-bold text-foreground">{movementType.name}</h1>
          ...
          {movementType.description && (
            <p className="text-sm text-muted-foreground">{movementType.description}</p>
          )}
        </div>
