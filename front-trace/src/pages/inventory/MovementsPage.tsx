...
const TYPE_CONFIG: Record<string, { label: string; icon: React.ElementType; color: string }> = {
  ...
  waste: { label: 'Merma', icon: Trash2, color: 'text-muted-foreground bg-secondary' },
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
      <div className="w-full max-w-lg bg-card rounded-3xl shadow-2xl p-6">
        <h2 className="text-lg font-bold text-foreground mb-4">Registrar Movimiento</h2>
            ...
            {MODAL_TABS.map(t => (
              <button
                ...
                className={cn(
                  ...
                  type === t.key ? `${t.color} text-white` : 'bg-secondary text-muted-foreground hover:bg-slate-200',
                )}
              >
                {t.label}
              </button>
  ...
  return (
    <div className="bg-card rounded-2xl border border-amber-200  overflow-hidden">
      ...
      <div className="divide-y divide-slate-50">
        {items.map(mv => (
          <div key={mv.id} className="flex items-center justify-between px-5 py-3 hover:bg-muted">
            <div className="flex items-center gap-4">
              ...
              <span className="text-sm font-bold text-foreground">{Number(mv.quantity).toFixed(2)}</span>
        ...
        {['', ...Object.keys(TYPE_CONFIG)].map(t => (
          <button
            ...
            className={cn(
              ...
              typeFilter === t ? 'bg-primary text-white' : 'bg-secondary text-muted-foreground hover:bg-slate-200',
            )}
          >
        ...
        {isLoading ? (
          ...
          <div className="space-y-3 p-4 md:hidden">
            {data.items.map(mv => {
              ...
              return (
                <div key={mv.id} className="rounded-xl border border-border bg-card p-4 space-y-2">
                  <div className="flex items-center justify-between">
                    {dynType ? (
                      ...
                      <span className={cn('flex items-center gap-1.5 rounded-full px-2 py-0.5 text-xs font-semibold w-fit', cfg?.color ?? 'bg-secondary text-muted-foreground')}>
                        <Icon className="h-3 w-3" />
                        {label}
                      </span>
                    )}
              ...
              {data.items.map(mv => {
                ...
                return (
                  <tr key={mv.id} className="hover:bg-muted">
                    ...
                    <td className="px-4 py-3 font-bold text-foreground">{Number(mv.quantity).toFixed(2)}</td>
                    <td className="px-4 py-3">
                      {mv.from_warehouse_id ? (
