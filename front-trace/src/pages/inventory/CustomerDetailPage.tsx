...
export function CustomerDetailPage() {
  ...
  async function doSubmitAddPrice() {
    const form = addPriceFormRef.current
    if (!form || !id) return
  ...
  const { formRef: addPriceFormRef, handleSubmit: validateAndSubmitAddPrice } = useFormValidation(doSubmitAddPrice)
          ...
          <div className="bg-card rounded-2xl shadow-2xl w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
            ...
            <form ref={editFormRef} onSubmit={validateAndSubmitEdit} noValidate className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div><label className="block text-xs font-medium text-muted-foreground mb-1">NIT / Número de identificación *</label><input name="tax_id" required defaultValue={customer.tax_id ?? ''} placeholder="Ej: 900123456" className="w-full px-3 py-2 text-sm border border-border rounded-xl focus:ring-2 focus:ring-ring/20 focus:border-ring outline-none" /></div>
