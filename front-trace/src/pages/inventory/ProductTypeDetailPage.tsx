...
import {
  ChevronRight, Tag, Info, Database, Plus, Pencil, Trash2, X, ImageIcon, Link2,
} from 'lucide-react'
import {
  useProductTypes, useUpdateProductType,
  useCustomFields, useCreateCustomField, useUpdateCustomField, useDeleteCustomField,
  useLocations,
} from '@/hooks/useInventory'
import { useConfirm } from '@/store/confirm'
import { cn } from '@/lib/utils'
import type { CustomField, FieldType } from '@/types/inventory'
...
function InfoTab({ productType }: { productType: { id: string; name: string; slug: string; description: string | null; color: string | null; requires_qc?: boolean; dispatch_rule?: 'fifo' | 'fefo' | 'lifo'; entry_rule_location_id?: string | null } }) {
  const update = useUpdateProductType()
  const { data: locations = [] } = useLocations()
  const [form, setForm] = useState({
    name: productType.name,
    description: productType.description ?? '',
    color: productType.color ?? '#6366f1',
    dispatch_rule: (productType.dispatch_rule ?? 'fifo') as 'fifo' | 'fefo' | 'lifo',
    requires_qc: productType.requires_qc ?? false,
    entry_rule_location_id: productType.entry_rule_location_id ?? '',
  })
...
function FieldsTab({ productTypeId }: { productTypeId: string }) {
  // Fields belonging to THIS product type
  const { data: fields = [], isLoading } = useCustomFields(productTypeId)
  // ALL system fields (no filter) — for the reuse picker
  ...
  // Fields available for reuse: system fields not already in this product type
  const myFieldKeys = new Set(fields.map(f => f.field_key))
  const reusableFields = allFields.filter(f =>
    f.product_type_id !== productTypeId && !myFieldKeys.has(f.field_key),
  )
  ...
  async function reuseField(source: CustomField) {
            ...
            <div className="grid gap-1.5 max-h-48 overflow-y-auto">
              {reusableFields.map(f => (
                <button key={f.id}
                  ...
                  className="flex items-center gap-3 rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-left hover:border-indigo-300 hover:bg-indigo-50/30 transition-colors disabled:opacity-50">
...
export function ProductTypeDetailPage() {
  ...
  const productType = types.find(t => t.id === id)
  if (!productType) return <Navigate to="/inventario/configuracion/tipos-producto" replace />
