  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">Ordenes de venta que requieren aprobación por monto</p>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <div className="bg-card rounded-2xl border border-border/60 p-4 text-center">
          <p className="text-xs font-bold text-muted-foreground uppercase">Urgentes (&gt;4h)</p>
        ...
        <div className="bg-card rounded-2xl border border-border/60 p-4 text-center">
          <p className="text-xs font-bold text-muted-foreground uppercase">Demoradas (1-4h)</p>
        ...
        <div className="bg-card rounded-2xl border border-border/60 p-4 text-center">
          <p className="text-xs font-bold text-muted-foreground uppercase">Recientes (&lt;1h)</p>
      ...
      {isLoading ? (
        ...
        <div className="bg-card rounded-2xl border border-border/60  p-12 text-center">
          ...
          <p className="text-lg font-semibold text-foreground">Sin aprobaciones pendientes</p>
          <p className="text-sm text-muted-foreground mt-1">Todas las ordenes están al día</p>
        </div>
      ) : (
              ...
              {orders.map((o: SalesOrder) => {
                ...
                return (
                  <tr key={o.id} className="hover:bg-muted/60 cursor-pointer" onClick={() => navigate(`/inventario/ventas/${o.id}`)}>
                    ...
                    <td className="px-6 py-3 text-foreground">{o.customer_name ?? o.customer_id.slice(0, 8)}</td>
                    <td className="px-6 py-3 text-right font-mono font-bold">${o.total.toLocaleString('es-CO')} {o.currency}</td>
                    <td className="px-6 py-3 text-muted-foreground text-xs">{o.created_by ?? '--'}</td>
                    <td className="px-6 py-3">
                      <span className="text-xs font-medium text-muted-foreground">{fmtWait(o.approval_requested_at)}</span>
      ...
      {rejectTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
          <div className="w-full max-w-md bg-card rounded-3xl shadow-2xl p-6">
            ...
            <textarea value={rejectReason} onChange={e => setRejectReason(e.target.value)} rows={3} className="w-full rounded-xl border border-border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-400" placeholder="Motivo del rechazo..." />
            <div className="flex justify-end gap-3 mt-4">
              <button onClick={() => setRejectTarget(null)} className="px-4 py-2 text-sm font-medium text-muted-foreground">Cancelar</button>
