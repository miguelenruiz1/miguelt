...
const VERIFICATION_BADGE: Record<string, { label: string; color: string }> = {
  unverified: { label: 'Sin verificar', color: 'bg-muted text-muted-foreground border-border' },
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-sm font-semibold text-foreground">Cadena de suministro</p>
          <p className="text-xs text-muted-foreground">EUDR Art. 9.1.e-f — Cada actor desde productor hasta importador</p>
        </div>
      ...
      {showForm && (
        <form onSubmit={handleAdd} className="rounded-xl border border-primary/30 bg-primary/5 p-4 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div>
              <label className="text-xs font-medium text-muted-foreground mb-1 block">NIT / Tax ID</label>
              <input value={form.actor_tax_id ?? ''} onChange={e => set('actor_tax_id', e.target.value)}
                className="w-full rounded-lg border border-border px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
            </div>
            <div>
              <label className="text-xs font-medium text-muted-foreground mb-1 block">EORI (si UE)</label>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className="text-xs font-medium text-muted-foreground mb-1 block">Fecha handoff</label>
            ...
            <div>
              <label className="text-xs font-medium text-muted-foreground mb-1 block">Direccion</label>
              <input value={form.actor_address ?? ''} onChange={e => set('actor_address', e.target.value)}
                className="w-full rounded-lg border border-border px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
            </div>
          </div>
          <div className="flex justify-end gap-2">
            <button type="button" onClick={() => setShowForm(false)} className="rounded-lg px-3 py-1.5 text-xs text-muted-foreground hover:bg-secondary">Cancelar</button>
      ...
      {nodes.length === 0 ? (
        <div className="rounded-xl border-2 border-dashed border-border p-8 text-center">
          ...
          <p className="text-sm font-medium text-muted-foreground mb-1">Sin actores en la cadena</p>
          <p className="text-xs text-muted-foreground">Agrega productores, recolectores, exportadores e importadores.</p>
        </div>
      ) : (
        <div className="relative">
          {nodes.map((node, i) => {
            const cfg = ROLE_CONFIG[node.role] ?? { label: node.role, icon: User, color: 'bg-muted text-muted-foreground border-border' }
            const Icon = cfg.icon
            ...
            return (
              <div key={node.id}>
                <div className="flex items-start gap-3 rounded-xl border border-border bg-card px-4 py-3">
                  {/* Sequence number */}
                  <div className="flex h-8 w-8 items-center justify-center rounded-full bg-secondary text-xs font-bold text-muted-foreground shrink-0 mt-0.5">
                    {node.sequence_order}
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <p className="text-sm font-semibold text-foreground">{node.actor_name}</p>
                    ...
                    {node.actor_address && <p className="text-xs text-muted-foreground mt-0.5">{node.actor_address}</p>}
                    {node.notes && <p className="text-xs text-muted-foreground italic mt-0.5">{node.notes}</p>}
                  </div>

                  {/* Actions */}
                  <button onClick={() => handleDelete(node.id)} disabled={deleteNode.isPending}
                    className="rounded-lg p-1.5 text-muted-foreground hover:text-red-500 hover:bg-red-50 transition-colors disabled:opacity-50 shrink-0">
