...
export default function DocumentUploader({ documents, isLoading, onAttach, onDetach, isPending }: Props) {
  ...
  if (isLoading) {
    return <div className="text-sm text-muted-foreground py-6 text-center">Cargando documentos...</div>
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-sm font-semibold text-foreground">Documentos de evidencia</p>
          <p className="text-xs text-muted-foreground">{documents.length} documento{documents.length !== 1 ? 's' : ''} adjunto{documents.length !== 1 ? 's' : ''}</p>
        </div>
      ...
      {documents.length === 0 ? (
        ...
        >
          ...
          <p className="text-sm font-medium text-muted-foreground mb-1">Sin documentos adjuntos</p>
          <p className="text-xs text-muted-foreground">Haz click para abrir la biblioteca de media y seleccionar o subir archivos</p>
        </button>
      ) : (
          ...
          {documents.map(doc => {
            ...
            return (
              <div key={doc.id} className="flex items-center gap-3 rounded-xl border border-border bg-card px-4 py-3">
                ...
                {isImg && fullUrl ? (
                  <a href={fullUrl} target="_blank" rel="noopener noreferrer" className="shrink-0">
                    <img
                      ...
                      className="h-10 w-10 rounded-lg object-cover border border-border hover:ring-2 hover:ring-primary/30 transition-all"
                    />
                  </a>
                ) : (
                  <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-muted shrink-0">
                    {isImg ? <FileImage className="h-4 w-4 text-purple-400" /> : <FileText className="h-4 w-4 text-muted-foreground" />}
                  </div>
                )}
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-foreground truncate">{doc.filename ?? 'Archivo'}</p>
                ...
                {doc.file_hash && (
                  <span className="text-[10px] font-mono text-muted-foreground shrink-0" title={doc.file_hash}>
