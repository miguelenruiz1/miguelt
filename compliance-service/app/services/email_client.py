"""Email client for compliance-service notifications.

Mirrors the shape of subscription-service's email_client.py — sends via
Resend, resolving per-tenant credentials from user-service (which owns the
`email_provider_configs` table). Falls back to env RESEND_API_KEY +
RESEND_FROM_EMAIL for local dev / Cloud Run.

Retries 3 times with exponential backoff (0.5s, 1s, 2s).

Kept as a separate module (instead of a shared lib) to avoid introducing
cross-service imports; the surface area is small and the two copies can be
